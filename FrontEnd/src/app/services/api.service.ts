...
export class ApiService {
  ...
  // Nuovi metodi per admin
  getRichiesteInAttesa(): Observable<TesseraLibreria[]> {
    return this.http.get<TesseraLibreria[]>(`${this.baseUrl}/api/tessere/admin/richieste`);
  }

  approvaTessera(richiestaId: number, note?: string): Observable<any> {
    const body = note ? { note } : {};
    return this.http.put(`${this.baseUrl}/api/tessere/admin/${richiestaId}/approva`, body);
  }

  rifiutaTessera(richiestaId: number, motivo: string): Observable<any> {
    return this.http.put(`${this.baseUrl}/api/tessere/admin/${richiestaId}/rifiuta`, { motivo });
  }
