        ...
        <div class="tipologie-grid">
          @for (tipologia of tipologieTessere; track tipologia.id) {
            <div class="tipologia-card" [class.featured]="tipologia.nome === 'Studente'">
			  ...
			  <button (click)="debugAuth()" class="btn btn-secondary">🔍 Debug Auth</button>
...
export class TessereComponent implements OnInit {
  ...
  // Aggiungi questo codice TEMPORANEO nel componente tessere per debug
  // Metti questo metodo nel TessereComponent e chiamalo prima di richiedere la tessera
  ...
  debugAuth(): void {
    ...
    // Test chiamata API protetta
    console.log('🧪 Test chiamata API protetta...');
    this.apiService.getCurrentUser().subscribe({
      next: (user) => {
        console.log('✅ Chiamata API riuscita:', user);
      },
      error: (error) => {
  ...
  // Aggiungi questo bottone nel template per testare:
  // <button (click)="debugAuth()" class="btn btn-secondary">🔍 Debug Auth</button>
