...
@Component({
  ...
  <div class="admin-container">
        ...
        <!-- Modal Dettagli Tessera/Richiesta -->
...
export class AdminDashboardComponent implements OnInit {
  ...
  utenti: Utente[] = [];
  richiesteInAttesa: TesseraLibreria[] = []; // 🆕 NUOVO
  ...
  selectedRichiesta: TesseraLibreria | null = null; // 🆕 NUOVO
  
  // Dropdown
  showDropdown: number | null = null;

  // 🆕 NUOVO: Stato per operazioni di approvazione/rifiuto
  ...
  loadData(): void {
    ...
    // 🆕 NUOVO: Carica richieste in attesa
    this.apiService.getRichiesteInAttesa().subscribe({
      next: (richieste) => {
        this.richiesteInAttesa = richieste;
        console.log('Richieste in attesa caricate:', richieste.length);
      },
      error: (error) => {
        console.error('Errore caricamento richieste:', error);
        // Non bloccare il caricamento se le richieste falliscono
      }
    });
  ...
  // 🆕 NUOVI METODI: Gestione Richieste Tessere
  
  approvaTessera(richiesta: TesseraLibreria): void {
    const note = prompt(`Approvi la richiesta di tessera "${richiesta.tipologia.nome}" per ${richiesta.utente.nome} ${richiesta.utente.cognome}?\n\nNote approvazione (opzionale):`, 'Richiesta approvata dall\'amministratore');
  ...
  rifiutaTessera(richiesta: TesseraLibreria): void {
    const motivo = prompt(`Rifiuti la richiesta di tessera "${richiesta.tipologia.nome}" per ${richiesta.utente.nome} ${richiesta.utente.cognome}?\n\nMotivo del rifiuto (richiesto):`, '');
