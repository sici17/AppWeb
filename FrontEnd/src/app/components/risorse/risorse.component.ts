...
export class RisorseComponent implements OnInit {
  ...
  confermaPrenotazione(): void {
    if (!this.selectedRisorsa || !this.dataInizio) return;
    
    this.confirmingBooking = true;
    
    // ✅ Risolto: Interface specifica per la richiesta
    const prestitoData: CreatePrestitoRequest = {
      risorsa: { id: this.selectedRisorsa.id },
      dataInizio: this.dataInizio,
      dataScadenza: this.calculateEndDate(this.dataInizio),
      stato: 'ATTIVO'
    };

    // ✅ Cast esplicito per TypeScript
    this.apiService.createPrestito(prestitoData as any).subscribe({
      next: (prestito) => {
        this.showFeedback('Prenotazione effettuata con successo!', 'success');
        this.closeBookingModal();
        this.loadUserCredits(); // Ricarica i crediti
      ...
      error: (error) => {
        ...
        if (error.status === 412) { // PRECONDITION_FAILED
          errorMessage = 'Crediti insufficienti';
        } else if (error.status === 400) {
          errorMessage = 'Risorsa non disponibile o dati non validi';
  ...
  private calculateEndDate(startDate: string): string {
    ...
    end.setDate(start.getDate() + 14); // 14 giorni di prestito
