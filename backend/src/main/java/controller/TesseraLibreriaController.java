...
import repositories.TesseraLibreriaRepository;
import services.*;
...
public class TesseraLibreriaController {
 ...
 // Aggiungi questi endpoint nel TesseraLibreriaController
    ...
    public ResponseEntity<?> getRichiesteInAttesa() {
        try {
            List<TesseraLibreria> richieste = null;
            try {
                richieste = TesseraLibreriaRepository
                        .findByStato(TesseraLibreria.StatoTessera.RICHIESTA_PENDING);
            } catch (Exception e) {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Errore caricamento richieste");
        }
    }
    ...
    public ResponseEntity<?> approvaTessera(@PathVariable int richiestaId, 
        ...
        try {
            ...
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Errore durante l'approvazione");
        }
    }
    ...
    public ResponseEntity<?> rifiutaTessera(@PathVariable int richiestaId,
        ...
        try {
            ...
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Errore durante il rifiuto");
        }
    }
